...
if [ -z "$OSIM_NGINX_PROXY_SPLUNK" ]; then
    exit 0;
fi
...
EOF
cat <<EOF >>/tmp/osim-nginx-proxy.conf
    proxy_set_header Authorization "Splunk $OSIM_SPLUNK_TOKEN";
    #proxy_ssl_trusted_certificate /tmp/Proxy-CA.crt;
    proxy_ssl_session_reuse off; # try off if connection issues
