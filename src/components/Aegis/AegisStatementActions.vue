...
const {
  ...
  sendFeedback,
  suggestStatement,
} = useAegisSuggestion(props.aegisContext as AegisSuggestionContextRefs, modelValue as Ref<null | string>, 'statement');
...
const suggestionTooltip = computed(() => {
  if (!hasAppliedSuggestion.value || !suggestionDetails.value) return 'Suggest Statement via AEGIS-AI';
