...
const suggestionTooltip = computed(() => {
  if (hasAppliedSuggestion.value && statementSuggestionDetails.value?.explanation) {
    const title = isStatementAIBot.value ? 'Generated by Aegis-AI-Bot' : 'Generated by Aegis-AI';
    return `${title}\n\n${statementSuggestionDetails.value.explanation}`;
  }
  const aiBotTooltip = getAIBotTooltip('statement');
