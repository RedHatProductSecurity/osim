...
describe('aegisDescriptionActions', () => {
  ...
  it('displays correct tooltip when suggestion is applied', () => {
    ...
    // Tooltip should only contain title and explanation
    expect(tooltip).toContain('Generated by Aegis-AI');
    expect(tooltip).toContain('Test explanation for description');
    // Should NOT contain value, confidence, or tools
    expect(tooltip).not.toContain(longDescription.substring(0, 100));
    expect(tooltip).not.toContain('0.88');
    expect(tooltip).not.toContain('analyzer, parser');
  });
});
