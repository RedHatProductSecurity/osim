...
describe('multiFlawTracker', () => {
  ...
  describe('paste handling', () => {
    ...
    it('should handle comma-separated CVEs on paste', async () => {
      const wrapper = await createWrapper();

      const input = wrapper.find('input[type="text"]');

      const pasteData = 'CVE-2024-0001, CVE-2024-0002, CVE-2024-0003';
      ...
      expect(addFlawMock).toHaveBeenCalledWith('CVE-2024-0001');
      expect(addFlawMock).toHaveBeenCalledWith('CVE-2024-0002');
      expect(addFlawMock).toHaveBeenCalledWith('CVE-2024-0003');
      expect(addFlawMock).toHaveBeenCalledTimes(3);
    });

    it('should handle newline-separated CVEs on paste', async () => {
      const wrapper = await createWrapper();

      const input = wrapper.find('input[type="text"]');

      const pasteData = 'CVE-2024-0001\nCVE-2024-0002\nCVE-2024-0003';
      const clipboardData = new DataTransfer();
      clipboardData.setData('text', pasteData);
    ...
    it('should handle space-separated CVEs on paste', async () => {
      const wrapper = await createWrapper();
    ...
    it('should handle mixed separators on paste', async () => {
      const wrapper = await createWrapper();
      ...
      expect(pasteEvent.defaultPrevented).toBe(true);
      expect(addFlawMock).toHaveBeenCalledTimes(4);
    });

    it('should validate each pasted entry and skip invalid ones', async () => {
      const wrapper = await createWrapper();

      const input = wrapper.find('input[type="text"]');

      const pasteData = 'CVE-2024-0001, invalid-id, CVE-2024-0002';
      const clipboardData = new DataTransfer();
      clipboardData.setData('text', pasteData);
      ...
      expect(addFlawMock).toHaveBeenCalledWith('CVE-2024-0001');
      expect(addFlawMock).toHaveBeenCalledWith('CVE-2024-0002');
      expect(addFlawMock).not.toHaveBeenCalledWith('invalid-id');
      expect(addFlawMock).toHaveBeenCalledTimes(2);
    });
    ...
    it('should handle paste with empty entries', async () => {
      const wrapper = await createWrapper();
