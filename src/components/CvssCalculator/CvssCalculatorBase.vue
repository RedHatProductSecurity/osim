...
const emit = defineEmits<{
  'blur': [FocusEvent];
  'change:cvss_score': [ZodAffectCVSSType[]];
}>();
...
watch(() => cvssEntity.value, () => {
  if (!equals(props.cvssEntity.cvss_scores, cvssEntity.value.cvss_scores)) {
    emit('change:cvss_score', cvssEntity.value.cvss_scores);
  }
}, { deep: true });
