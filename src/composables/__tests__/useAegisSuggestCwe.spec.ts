...
import { buildCveAnalysisContext } from '@/composables/aegis/useAegisSuggestionContext';
...
function createContext(overrides?: Partial<Parameters<typeof buildCveAnalysisContext>[0]>) {
  const ctx = {
...
it('calls service with expected payload when suggesting', async () => {
  ...
  expect(analyzeMock).toHaveBeenCalledWith(expect.objectContaining({
    feature: 'suggest-cwe',
    ...buildCveAnalysisContext(context as AegisSuggestionContextRefs),
  }));
});
