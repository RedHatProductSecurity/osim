...
import {
  useAegisSuggestionContextFromFlaw,
  buildCveAnalysisContext,
...
describe('useAegisSuggestionContext', () => {
  it('maps flaw ref fields to context refs', async () => {
    ...
    const [ctx] = withSetup(() => useAegisSuggestionContextFromFlaw(flaw), []);
  ...
  it('builds request context with expected keys and fallbacks', async () => {
    ...
    const payload = buildCveAnalysisContext(ctx);
