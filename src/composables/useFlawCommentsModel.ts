    ...
    getJiraComments(flaw.value.task_key)
      ...
      .catch((res) => {
        if (res?.response?.status === 404) {
          console.error('useFlawCommentsModel::loadInternalCommens() Error loading Jira comments: Task id not found.');
        } else {
          console.error('useFlawCommentsModel::loadInternalCommens() Error loading Jira comments');
        }
        internalCommentsAvailable.value = false;
      })
      .finally(() => {
