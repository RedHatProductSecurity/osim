...
export async function getAffects(cveOrUuid: string) {
  ...
  const firstResponse = await osidbFetch({
    method: 'get',
    url: `/osidb/api/${osimRuntime.value?.flags?.affectsV2 ? 'v2beta' : 'v1'}/affects`,
    params: {
      [field]: cveOrUuid,
      limit: pageSize,
    },
...
export async function putAffects(affectObjects: any[]) {
  return osidbFetch({
    method: 'put',
    url: `/osidb/api/${osimRuntime.value?.flags?.affectsV2 ? 'v2beta' : 'v1'}/affects/bulk`,
    data: affectObjects,
  })
...
export async function putAffectCvssScore(
  ...
  const apiVersion = isV2 ? 'v2beta' : 'v1';
